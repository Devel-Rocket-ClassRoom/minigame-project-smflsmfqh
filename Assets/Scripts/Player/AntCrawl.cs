...
public class AntCrawl : MonoBehaviour
{
    ...
    [SerializeField]
    private float _surfaceHoldRange = 0.6f;
    ...
    [SerializeField]
    private float _snapOffset = 0.25f;

    [Header("이탈 점프")]
    [Tooltip("Space 이탈 시 표면 법선 방향으로 가해지는 힘")]
    [SerializeField]
    private float _exitJumpForce = 4f;
    ...
    [SerializeField]
    private int _lostSurfaceThreshold = 5;

    [Tooltip("표면 snap velocity 최대치 — 너무 크면 진동 발생")]
    [SerializeField]
    private float _snapVelMax = 2f;
    ...
    private const int k_PlayerLayer   = 3;
    private const int k_ObstacleLayer = 7;
    ...
    private void FixedUpdate()
    {
        ...
        if (_pendingSnapOnEntry)
        {
            ...
            float dist = Vector3.Dot(
                transform.position - _lastSurfaceHit.point,
    ...
    private void TryEnterCrawl()
    {
        ...
        {
            Debug.Log(
                $"[AntCrawl] 이탈 쿨다운 중: {_exitCooldown - (Time.time - _exitCrawlTime):F2}s 남음"
            );
            return;
        }
        ...
        Vector3 moveDir = (
            transform.forward * InputDir.y + transform.right * InputDir.x
        ).normalized;

        Debug.DrawRay(transform.position, moveDir * _detectRange, Color.cyan);
        Debug.Log(
            $"[AntCrawl] SphereCast 시도: origin={transform.position}, dir={moveDir:F2}, range={_detectRange}, mask={_crawlMask.value}"
        );
        ...
        Debug.Log(
            $"[AntCrawl] ✅ 벽 감지 성공: normal={hit.normal:F2}, normalDotUp={normalDotUp:F2} (0.5 미만이면 진입)"
        );
    ...
    private void EnterCrawl(RaycastHit hit)
    {
        ...
        Physics.IgnoreLayerCollision(k_PlayerLayer, k_ObstacleLayer, true);
    ...
    private void ExitCrawl(Vector3 exitVelocity)
    {
        ...
        _rb.useGravity = true;
        Physics.IgnoreLayerCollision(k_PlayerLayer, k_ObstacleLayer, false);
    ...
    private void AlignToSurface(Vector3 surfaceNormal)
    {
        ...
        _rb.MoveRotation(
            Quaternion.Slerp(transform.rotation, targetRot, _alignSpeed * Time.fixedDeltaTime)
        );
    }
