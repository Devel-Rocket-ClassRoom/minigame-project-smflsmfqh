...
public class LeaderboardUI : MonoBehaviour
{
    ...
    private async UniTaskVoid Start()
    {
        ...
        statusText.text = "[LeaderboardUI] 실시간 동기화 중...";
        await LeaderboardManager.Instance.StartRealtimeListener(topCount);
    }

    private async UniTaskVoid LoadAndDisplayLeaderboardAsync()
    {
        statusText.text = "[LeaderboardUI] 실시간 동기화 로딩 중...";
    ...
    private void OnRealtimeToggleChanged(bool isOn)
    {
        if (isOn)
        {
            ...
            statusText.text = "[LeaderboardUI] 실시간 동기화 켜짐";
            isRealtimeEnabled = true;
        }
        else
        {
            LeaderboardManager.Instance.StopRealtimeListener();
            statusText.text = "[LeaderboardUI] 실시간 동기화 꺼짐";
